...
```powershell
npm install
npm run velopack:pack -- -Version 0.2.4
```
...
```powershell
npm run velopack:pack -- -Version 0.2.4 -CleanOutput
```
...
```powershell
npm run velopack:pack -- -Version 0.2.4 -Msi
```

The MSI default scope is `PerUser`. To change it at package time:

```powershell
npm run velopack:pack -- -Version 0.2.4 -Msi -InstallScope PerMachine
```
