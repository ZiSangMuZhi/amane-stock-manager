...
```powershell
npm install
npm run velopack:pack -- -Version 0.1.12
```
...
```powershell
npm run velopack:pack -- -Version 0.1.12 -CleanOutput
```
...
```powershell
npm run velopack:pack -- -Version 0.1.12 -GithubRepoUrl "https://github.com/ZiSangMuZhi/amane-stock-manager" -PublishGitHub
```
...
```powershell
npm run velopack:pack -- -Version 0.1.12 -Msi
```

The MSI default scope is `PerUser`. To change it at package time:

```powershell
npm run velopack:pack -- -Version 0.1.12 -Msi -InstallScope PerMachine
```
