...
```powershell
npm install
npm run velopack:pack -- -Version 0.1.16
```
...
```powershell
npm run velopack:pack -- -Version 0.1.16 -CleanOutput
```
...
```powershell
npm run velopack:pack -- -Version 0.1.16 -GithubRepoUrl "https://github.com/ZiSangMuZhi/amane-stock-manager" -PublishGitHub
```
...
```powershell
npm run velopack:pack -- -Version 0.1.16 -Msi
```

The MSI default scope is `PerUser`. To change it at package time:

```powershell
npm run velopack:pack -- -Version 0.1.16 -Msi -InstallScope PerMachine
```
