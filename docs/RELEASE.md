...
```powershell
npm install
npm run velopack:pack -- -Version 0.1.15
```
...
```powershell
npm run velopack:pack -- -Version 0.1.15 -CleanOutput
```
...
```powershell
npm run velopack:pack -- -Version 0.1.15 -GithubRepoUrl "https://github.com/ZiSangMuZhi/amane-stock-manager" -PublishGitHub
```
...
```powershell
npm run velopack:pack -- -Version 0.1.15 -Msi
```

The MSI default scope is `PerUser`. To change it at package time:

```powershell
npm run velopack:pack -- -Version 0.1.15 -Msi -InstallScope PerMachine
```
