...
function itemRows(inventory: InventoryFile): Array<Record<string, string | number>> {
  return sortedItems(inventory).map((item) => ({
    ...
    purchasePriceAmount: item.priceAmount ?? '',
    salePriceAmount: item.salePriceAmount ?? '',
    priceCurrency: item.priceCurrency,
    stockValue: item.priceAmount === null ? '' : item.priceAmount * item.quantityOnHand,
    purchaseStockValue: item.priceAmount === null ? '' : item.priceAmount * item.quantityOnHand,
    saleStockValue: item.salePriceAmount === null ? '' : item.salePriceAmount * item.quantityOnHand,
    grossProfitValue:
      item.priceAmount === null || item.salePriceAmount === null
        ? ''
